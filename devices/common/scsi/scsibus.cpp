...
void ScsiBus::attach_scsi_devices(const std::string bus_suffix)
{
    ...
    std::string bluescsi_path = GET_STR_PROP("bluescsi_dir" + bus_suffix);
    if (!bluescsi_path.empty()) {
        std::string bluescsi_send_path = GET_STR_PROP("bluescsi_send_dir" + bus_suffix);
        if (bluescsi_send_path.empty()) {
            bluescsi_send_path = bluescsi_path;
        }
        for (scsi_id = 0; scsi_id < SCSI_MAX_DEVS && this->devices[scsi_id]; scsi_id++) {}

        if (scsi_id < SCSI_MAX_DEVS) {
            std::string scsi_device_name = "BlueScsi" + bus_suffix + "," +
                                            std::to_string(scsi_id);
            BlueScsi *scsi_device = new BlueScsi(scsi_device_name,
                                                 scsi_id,
                                                 bluescsi_path,
                                                 bluescsi_send_path);
            gMachineObj->add_device(scsi_device_name,
